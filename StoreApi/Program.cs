...
void MapStaffEndpoints(WebApplication app, string tag)
{
    app.MapGet("api/staffs", (IStaffService service)
        => { return service.ReadAll(); }).WithTags(tag);
    app.MapGet("api/staffs/{key}", (IStaffService service, string key)
        => { return service.Read(key); }).WithTags(tag);
    app.MapPost("api/staffs", (IStaffService service, StaffCreateReq req)
        => { return service.Create(req); }).WithTags(tag);
    app.MapPut("api/staffs", (IStaffService service, StaffUpdateReq req)
        => { return service.Update(req); }).WithTags(tag);
    app.MapDelete("api/staffs/{key}", (IStaffService service, string key)
        => { return service.Delete(key); }).WithTags(tag);
